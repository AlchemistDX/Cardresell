...
| 4 | Confirm the old Cert ID is retired, and check what happens to **already-issued tokens** | eBay portal | Secret rotation does not necessarily invalidate live tokens; assuming it does is how a rotation looks complete while the old credential still works. **ANSWERED 2026-09-10 22:05 from eBay's documentation — the caution was right: "Creating a new Cert ID does not affect existing user tokens." And the ordering consequence the plan did not have: the new Cert ID *cannot* revoke tokens created with the old one, so any revocation must happen WHILE THE OLD CERT ID IS STILL VALID. Letting it expire first destroys the ability to revoke. See `audit/ROTATION_RECHECK_2026-09-10.md` §7.** |
