...
| RV-10 containment mechanism | **EXECUTED for KV, verified by listing** — the second Redis database was created and the targeting is disjoint (Production vs Preview+Development), confirmed 2026-09-10 00:20. The earlier row read "designed, not executed" and is corrected. Cause and rejected alternatives retained in §4. **Corrected 2026-09-10 21:50 — one, not three.** `PRICECHARTING_API_TOKEN` is now **Production only** (narrowed per the D-RV-2 order; re-read from the CLI today). Blob is **closed by D-RV-4** — nothing reads either Blob variable and the photo path is IndexedDB, so there is no production photo storage for Preview to reach; cleanup is non-blocking. **`TURNSTILE_SECRET_KEY` remains broader than Production** and is tracked separately under **D-RV-3** (a distinct Preview *test* credential, not a narrowing), alongside older-deployment access under **Safeguard 2**. See §Redis reconciliation. |
...
> remedy. ~~**Three non-KV production resources remain reachable from
> Preview**~~ **Corrected 2026-09-10 21:50 — one, not three: Turnstile.**
> PriceCharting is now Production-only and Blob is closed by D-RV-4. Turnstile
> is tracked separately under D-RV-3, with older-deployment access under
> Safeguard 2. Either way, not RV-10.
