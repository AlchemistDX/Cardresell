...
| 6b | **PENDING — owner's Mac.** `node tools/verify-challenge.mjs` (corrected script, interactive terminal required, refuses piped input) with the **same** token saved in Vercel. **This is the only check that establishes which token production hashes.** **Correction recorded 2026-09-11:** an HTTP 200 with a `challengeResponse` body does **not** identify the token in use — I previously offered the pre-rebuild 200 as a baseline observation, and any reading of it as token evidence is **withdrawn**. Only the hash comparison establishes it. **Must PASS before step 7.** | operator shell |
