...
| # | Action | Surface |
| --- | --- | --- |
| **1c** | **DONE — 2026-09-11 00:58. Result: NOT DISPLAYED.** Owner inspected the eBay Production keyset, read-only, controls untouched. The portal **does not display** generation count, current-vs-grace, or any expiry date or status. Recorded as **not displayed**, which is a fact about the **portal's UI**, not about the keyset: it neither establishes that only one generation exists nor that none is in grace. The historical rotation count remains **unobtainable** from this surface; a prior rotation *attempt* is on record (`EBAY_OAUTH_TICKET.md` line 27), so **"at least the second"** remains the whole claim. No values were shared. | eBay developer portal |
| **1d** | **NOT RUNNABLE — blocked by 1c's result, 2026-09-11 00:58.** The comparison matches the Vercel project row against **portal-displayed generation values**. The portal displays **none**, so there is nothing to compare against and the tool is not run. `tools/compare-cert-generation.mjs` and its suite are retained for a portal that does display them. **Consequence, stated plainly: the generation currently configured in Vercel is UNKNOWN and cannot be identified by any step in this runbook.** Reading the Vercel row alone would show a value with **nothing to map it to**, so readback is pointless here and is **not** performed. **The runbook must not claim the configured generation was identified.** | — |
...
| **1f** | **Predecessor: UNKNOWN — recorded, not inferred.** With 1d not runnable, the generation this rotation supersedes **cannot be named**, and **no old-generation revocation route can be selected** for it. Record both as unknown. **Operational consequence:** the rotation proceeds as a **replacement whose predecessor is unidentified**. Any older generation that eBay still honours in grace stays **unaccounted for**, and this runbook cannot close that gap — it is a limit of the portal surface, not an oversight. If eBay's portal or support later exposes generation state, revisit before assuming the old value is dead. | rotation record |
