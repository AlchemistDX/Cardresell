...
| **1d** | **Compare the Vercel-stored Cert ID against those generations, locally: `node tools/compare-cert-generation.mjs`.** Hidden prompts, no echo, no file written, **no network request of any kind** — it authenticates nothing. Prints only the matched generation label, or `no match`. `EBAY_CERT_ID` is stored **`encrypted`, not `sensitive`** (`ROTATION_GATE_ANSWERED.md:71`), so the owner can reveal it for this comparison. **A match establishes which generation is configured. `no match` establishes only that it is none of the *displayed* generations — expired ones no longer shown can be neither ruled in nor out, so the configured generation stays UNKNOWN.** If readback is unavailable for any reason, it stays unknown; the rotation may replace it regardless, and **the runbook must not claim this step identified it**. | operator shell |
| **1e** | **Run the single token exchange on that exact pair: `node tools/verify-ebay-credential.mjs`.** Record **accepted**, **rejected**, or **unreachable**. **What each does and does not establish:** *accepted* — the supplied App ID + Cert ID pair is currently valid; *rejected* — that pair is not accepted, which is **not** an identification of a generation, since it is equally consistent with an expired credential, a mismatched pair, a transcription error, or another rejection cause; *unreachable* — neither, and it cannot license proceeding. **1e never substitutes for 1d.** An exchange describes the **supplied pair**; only 1d maps a value to a **generation**. | operator shell |
