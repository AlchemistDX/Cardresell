...
| # | Action | Surface |
| --- | --- | --- |
| **1c** | **Record the Cert ID generation state, before generating anything. Added 2026-09-10 23:40.** A prior Cert ID rotation is already on record (`EBAY_OAUTH_TICKET.md` line 27, published copy: *"Cert ID rotation performed — same 401 with the fresh cert"*), so this is at least the **second** rotation, not the first. That sentence is documentary evidence an attempt happened; it establishes **nothing about current portal state**. Record, **without values**: **(a)** how many Cert ID generations the portal now lists; **(b)** which is current, which is still in grace, and each one's expiry state; **(c)** which generation this rotation will supersede; **(d)** which old-generation revocation route applies to the superseded one. | eBay developer portal |
| **1d** | **Establish which generation Vercel's stored Cert ID actually is** — by running the planned token exchange (`node tools/verify-ebay-credential.mjs`, one exchange, hidden prompts, output redacted and verified by `tests/ebay-credential-check.mjs`). A `pass` establishes the stored value is **the currently accepted generation**; a `fail` establishes it is not. **Unreachable is neither** and cannot license proceeding. Without 1d, the rotation could supersede a predecessor that is not the one in production and leave an older generation unaccounted for. | operator shell |
