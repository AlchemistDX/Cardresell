...
| **1d** | **DONE — 2026-09-11 09:40. Result: NO MATCH.** Owner ran `compare-cert-generation.mjs` locally: the **Vercel project row** value of `EBAY_CERT_ID` **differs** from the Production Cert ID the portal currently shows. Complete-value equality, so this is not a near-miss or formatting artefact. **Establishes:** the two values are different. **Does NOT establish:** which is older, which generation either belongs to, or that the Vercel value is invalid — the portal exposes no generation numbering, and 1e passed on the **portal** pair, which says nothing about the Vercel row. | operator shell — complete |
| **1e** | **DONE — 2026-09-11 09:40. Result: ACCEPTED**, on the **portal-shown pair**. eBay issued an application token. **Establishes:** that pair is currently valid, and the "appears not enrolled for `client_credentials`" hypothesis is **disproved** — the grant works. **Does NOT establish:** that the deployed application uses that pair (at `9aaf326` production reads **neither** `EBAY_APP_ID` nor `EBAY_CERT_ID`), nor that the **Vercel-row** value is valid — the Vercel row was never exchanged, and per 1d it is a different value. | operator shell — complete |
| **1f** | **Predecessor: the portal-shown Cert ID; the Vercel row is a SEPARATE unaccounted value. Recorded 2026-09-11 09:40.** The rotation supersedes the Cert ID the portal showed as current on 2026-09-11 — validity established at 1e, generation number **not exposed** by the portal. **Newly established by 1d, and it is not a tidy result:** the value sitting in Vercel Production is a **third value**, neither the portal-current one nor identifiable as any generation. Whether eBay still honours it is **unknown and untested** (exchanging it would be a second live exchange; not performed). Record it as **an unaccounted credential value, status unknown** — do not assume it is dead, and do not assume it is live. No revocation route can be selected by number for either. | rotation record |
