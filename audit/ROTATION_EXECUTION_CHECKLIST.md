...
| # | Action | Surface |
| --- | --- | --- |
| **1c** | **Record the portal-visible Cert ID generations, before generating anything. Added 23:40, corrected 23:55.** Record, **without values**: each generation the portal **currently displays**, which is current, which is in grace, and each expiry state. **Scope limit, stated in the record itself:** this captures only what the portal displays. If expired generations are no longer listed, it **cannot establish the total historical rotation count**, and must not be written up as if it does. A prior rotation *attempt* is on record (`EBAY_OAUTH_TICKET.md` line 27, published copy), so this is **at least** the second — "at least" is the whole claim. | eBay developer portal |
| **1d** | **Compare the Vercel project's stored Cert ID against those generations, locally: `node tools/compare-cert-generation.mjs`.** Requires an **interactive TTY** and **refuses piped or redirected input**, the same rule as the live checker — piped input would land in shell history or a file. Hidden prompts, no echo, nothing written, **no network request of any kind**; it authenticates nothing. **Complete-value equality only** (full SHA-256, fixed-width comparison): it prints a matched generation label or `no match`, and **never a partial, prefix, or "closest" match**. `EBAY_CERT_ID` is stored **`encrypted`, not `sensitive`** (`ROTATION_GATE_ANSWERED.md:71`), so owner readback is available. **Scope, precisely:** this identifies the value on the **Vercel project's environment row** — **not** a credential embedded in the running production deployment. At `9aaf326` production **does not read `EBAY_CERT_ID` at all**, so the row is the only thing there is to identify. **A match identifies that row's generation. `no match` rules out only the *displayed* generations**, leaving the configured generation **unknown**; likewise if readback is unavailable. The rotation may replace it regardless, and **the runbook must not claim 1d identified it**. | operator shell |
| **1e** | **Run the single token exchange on that exact pair: `node tools/verify-ebay-credential.mjs`.** Record **accepted**, **rejected**, or **unreachable**. **accepted** — the supplied App ID + Cert ID pair is currently valid. **rejected** — **the supplied pair was rejected**, and **nothing more**. Stop the conclusion there. It does **not** establish that Vercel is misconfigured, **and a 1d Cert ID match does not upgrade it** — the **supplied App ID** could itself be wrong, and a rejection is equally consistent with an expired credential, a mismatched pair, a transcription error, or another rejection cause. **unreachable** — neither, and it cannot license proceeding. **1e never substitutes for 1d**: an exchange describes the **supplied pair**; only 1d maps a value to a **generation**. | operator shell |
| **1f** | From 1c–1e, record **which generation this rotation supersedes** and **which old-generation revocation route applies to it**. If 1d returned `no match` or could not run, record the predecessor as **unknown** rather than inferring one. | rotation record |
