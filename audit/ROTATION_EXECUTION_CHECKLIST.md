...
| **1e** | **Run the single token exchange on that exact pair: `node tools/verify-ebay-credential.mjs`.** Record **accepted**, **rejected**, or **unreachable**. **accepted** — the supplied App ID + Cert ID pair is currently valid. **rejected** — *this supplied pair was rejected*, and **nothing more**. It does **not** establish that Vercel is misconfigured **unless 1d first matched the supplied Cert ID to the Vercel row**; absent that, it is equally consistent with an expired credential, a mismatched pair, a transcription error, or another rejection cause. **unreachable** — neither, and it cannot license proceeding. **1e never substitutes for 1d**: an exchange describes the **supplied pair**; only 1d maps a value to a **generation**. | operator shell |
