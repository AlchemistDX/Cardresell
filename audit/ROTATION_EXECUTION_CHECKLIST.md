...
| **1d** | **RUNNABLE, with a narrowed conclusion. Corrected 07:24 — my "not runnable" entry is withdrawn.** Run `node tools/compare-cert-generation.mjs` in the owner's terminal: interactive TTY required, piped input refused, no echo, nothing written, **no network request**, **complete-value equality only** (full SHA-256, fixed width), never a partial or closest match. Enter the **Vercel project row** value (`EBAY_CERT_ID`, stored `encrypted` not `sensitive` — `ROTATION_GATE_ANSWERED.md:71`) and the **revealed Production Cert ID**, labelled simply `portal-current` since the portal supplies no generation number. **A match establishes exactly this: the Vercel row matches the Production Cert ID the portal currently shows.** It does **not** establish that value's **historical generation number**, and it does **not exclude another credential still honoured in grace**. **`no match`** establishes only that the row differs from the shown value. | operator shell |
...
| **1f** | **Record the predecessor at the precision available — no more.** With a 1d match, record: *"the rotation supersedes the Cert ID the portal showed as current on 2026-09-11; its generation number is not exposed by the portal."* With `no match`, or if 1d is not run, record the predecessor as **unknown**. Either way, **no old-generation revocation route can be selected by number**, and any credential eBay still honours in grace remains **unaccounted for** — a limit of the portal surface, recorded rather than inferred away. | rotation record |
