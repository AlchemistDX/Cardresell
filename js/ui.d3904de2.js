...
window._bulkQueue       = [];   // array of {file, objectUrl, scanUid} waiting to be processed
...
/* ── Scan-row identity ──────────────────────────────────────────────────────
   ...
   `rowId` is POSITIONAL (`bulk-row-<index>`) and is reassigned whenever the
   list is rebuilt -- bulkMergeDuplicates() renumbers every row. It is a DOM
   handle, and it is the wrong thing to hang a draft identity on: the same
   physical card can change rowId, and a rowId can later name a different card.
   `col-<entryId>` is no help either, because these rows are unsaved -- there is
   no collection entry yet, and creating one just to obtain an identity would
   make drafting a card silently add it to the Collection.

   So every captured photo is minted a `scanUid` at CAPTURE time, before the
   scan runs, and the uid travels on the queue item -> the result -> the merged
   result. Three properties follow, each one a rule this file is tested against
   (tests/bulk-batch-draft.mjs):

     1. A RETRY REUSES THE IDENTITY. bulkRetryRow() replaces the whole result
        object, so the uid is carried onto the replacement explicitly. A retry
        is the same intent for the same photo, so it must resolve to the same
        idempotency key and replay rather than create a second draft.
     2. TWO PHYSICAL COPIES STAY DISTINCT. Two photos of the same card are two
        uids, therefore two instances, two keys and two drafts -- which is
        correct, because they are two cards in hand. Merging them into one row
        with `qty: 2` does not collapse that: the merged row keeps BOTH uids in
        `copyUids`, one per physical copy, the same way _bulkSaveToCollection
        expands qty into qty separate entries.
     3. A UID IS NEVER RECYCLED ONTO A DIFFERENT CARD. Parked uids (a qty the
        seller lowered) stay parked on the row they belong to, so they can only
        ever be re-issued to the same card -- the case where replaying the
        earlier draft is the right answer rather than a duplicate.

   `inst_scan_<uid>` is the instance id, matching the `inst_col_<entryId>`
   vocabulary core.js already uses. Nothing here invents a second idempotency
   rule: the key is built by _crCreateIdemKey and the retry payload is held by
   _crCreateAttempt, both in core.js. */
window._bulkDraftSel      = [];                  // scanUids of rows the seller selected
window._bulkDraftOutcomes = Object.create(null); // per-copy uid -> outcome record
window._bulkDraftBusy     = false;
...
function openBulkScan() {
  ...
  window._bulkDraftSel      = [];
  window._bulkDraftOutcomes = Object.create(null);
  window._bulkDraftBusy     = false;
...
function bulkRapidSnap() {
  ...
  canvas.toBlob(blob => {
    ...
    // The identity is minted at capture, not at scan: it has to survive a
    // retry, and a retry rebuilds the result from this queue item.
    window._bulkQueue.push({ file, objectUrl, scanUid: _bulkNewScanUid() });
    _bulkRapidRenderStrip();
    _bulkRapidUpdateCounter();
  }, 'image/jpeg', 0.9);
}
...
function processBulkUploadFiles(input) {
  ...
  window._bulkQueue = files.map(f => ({ file: f, objectUrl: URL.createObjectURL(f), scanUid: _bulkNewScanUid() }));
...
async function _bulkScanOne(item, rowId, isRetry) {
  ...
  let result = {
    ...
    // never reach the draft path without an identity.
    scanUid: (item && item.scanUid) || _bulkNewScanUid(),
  };
  try {
    ...
    if (resp.status === 402) {
      ...
      if (data.card_name) {
        ...
        if (priceInfo) {
          ...
          // Provenance, stamped where the number is produced. A draft created
          // from this row declares `comp` only because THIS read happened; a
          // seller edit below rewrites it to `seller`. Neither is inferred at
          // create time, because an inferred provenance is a claim about where
          // a price came from that nobody checked.
          if (priceInfo.marketPrice != null) result.priceSource = 'comp';
...
function _bulkFinishQueue(total) {
  ...
  // Reveal the selection bar now that the list has settled. Nothing is
  // pre-selected: a batch of drafts is the seller's decision, and a screen
  // that arrives with forty cards ticked invites one tap they did not mean.
  _bulkDraftSyncSelectionUI();
}
...
function bulkMergeDuplicates() {
  ...
  (window._bulkResults || []).forEach(r => {
    ...
    const key = _bulkDupKey(r);
    if (!groups[key]) {
      // copyUids carries ONE identity per physical copy. Merging is a display
      // decision -- two photos are still two cards in hand, and
      // _bulkSaveToCollection already expands qty into qty separate entries.
      // Collapsing them onto a single identity here would make the second copy
      // replay the first copy's draft, and the seller would be left with one
      // draft for two cards with no indication the second went missing.
      const merged = Object.assign({}, r, {
        qty: r.qty || 1,
        copyUids: _bulkCopyUids(r).slice(0, r.qty || 1),
      });
      groups[key] = merged;
      order.push({ r, merged, isFirst: true });
    } else {
      // The uid list is authoritative and qty is DERIVED from it, in that
      // order. Bumping qty first is what the 2026-09-11 batch suite caught:
      // _bulkCopyUids repairs a short list against the new qty, so it minted a
      // filler uid for the copy that was about to be concatenated, and the
      // copy's real identity was then pushed past the qty slice and lost --
      // two cards in hand, one draft, no indication the second was dropped.
      // Each side is sliced to its own qty so a parked surplus uid (left by an
      // earlier qty reduction) cannot inflate the merged count.
      const uids = _bulkCopyUids(groups[key]).slice(0, groups[key].qty || 1)
        .concat(_bulkCopyUids(r).slice(0, r.qty || 1));
      groups[key].copyUids = uids;
      groups[key].qty = uids.length;
  ...
  // Selection follows the cards, not the rows. A row that disappears into a
  // merge must not silently deselect the copy the seller picked, so a group is
  // selected if ANY of the copies now inside it was selected before.
  const prevSel = new Set(window._bulkDraftSel || []);
  window._bulkResults = newResults;
  window._bulkDraftSel = newResults
    .filter(r => r && r.success && _bulkCopyUids(r).some(u => prevSel.has(u)))
    .map(r => r.scanUid);
...
function _bulkUpdateRow(rowId, result) {
  ...
  try { _bulkUpdatePackTracker(); } catch (e) {}
  // Same reasoning for the draft selection summary: qty edits, removals,
  // retries and merges all change what "N selected · M drafts" should say, and
  // they all repaint through here.
  try { _bulkDraftSyncSelectionUI(); } catch (e) {}
}
...
async function bulkEditPrice(rowId) {
  ...
  result.priceSource  = 'seller'; // and no longer a comp-derived number; see _bulkScanOne
  _bulkUpdateRow(rowId, result);
}
...
async function bulkRetryRow(rowId) {
  ...
  //
  // scanUid is carried across too. _bulkScanOne returns a NEW result object, so
  // without this the retried row would be minted a fresh identity and a draft
  // created before the retry would no longer replay -- the seller would get a
  // second draft for one card. Same photo, same intent, same identity.
  const item = {
    file: oldResult.file,
    objectUrl: oldResult.objectUrl,
    retryOf: oldResult.scanId || '',
    scanUid: oldResult.scanUid || '',
  };
  ...
  // Belt and braces on the identity: _bulkScanOne reads item.scanUid, and this
...
function bulkRemoveResult(rowId) {
  // Drop the row's selection with the row. Selection is keyed by scanUid, so a
  // stale entry could not resurrect a removed card -- but it would keep
  // inflating the "N selected" count the seller is reading.
  try {
    const gone = _bulkFindResult(rowId);
    if (gone && gone.scanUid) {
      window._bulkDraftSel = (window._bulkDraftSel || []).filter(u => u !== gone.scanUid);
    }
  } catch (e) {}
...
/* ═══════════════════════════════════════════════════════════════════════════
   BATCH DRAFTING FROM BULK / RAPID SCAN

   One selection, N listing drafts, one outcome per card. The scope boundary is
   deliberate and narrow: this creates DRAFTS. It does not publish, does not
   price, does not touch the Collection, and it ends where the single-card path
   already ends -- in Drafts, where the existing review and export screens take
   over. Nothing about the venue integration changes.

   THREE RULES THIS BLOCK IS BUILT AROUND

   1. THE SERVER OWNS THE CAP. _bulkDraftHeadroom() reads the seller's current
      draft count so the bar can say "room for 12 more" BEFORE forty photos are
      committed to. That number is advisory and is described as such: drafts can
      be created in another tab, on another device, or by a request already in
      flight, so it can be wrong by the time the batch reaches the cap. The only
      authority on whether a draft may exist is the create call's answer, and a
      refusal is shown on the row it belongs to -- see _bulkDraftClassify.

   2. EVERY CARD REPORTS ITS OWN RESULT. There is no batch-level "3 of 7
      failed". A seller who is told that has to work out WHICH three, and the
      most likely response is to run the batch again -- which is safe here only
      because of rule 3, and is still work nobody should have to do. Each row
      carries its own line, and cards after a cap refusal are marked "not
      attempted" rather than silently left blank.

   3. RE-RUNNING IS A REPLAY, NOT A SECOND DRAFT. Each physical copy has a
      stable identity (see the scan-row identity note at the top of this
      section), so its idempotency key is the same on every run. core.js already
      holds the create attempt and replays it; this block adds no second
      idempotency rule and no local "already done" bookkeeping that could
      disagree with the server. Pressing the button twice is therefore safe, and
      so is pressing it after fixing the two rows that failed.

   CONCURRENCY IS ONE ON PURPOSE. The scan queue runs three at a time because
   scans are independent. Creates are not: they draw down one shared cap, and
   parallel creates make it arbitrary which cards get the last slots -- the
   seller would see refusals scattered across rows they did not choose. Serial
   creates make the refusal boundary the seller's own order. (Separately: the
   REST-level results recorded for the KV store do not establish anything about
   application-level concurrency here, so they are not being leaned on.)
   ═══════════════════════════════════════════════════════════════════════════ */

function _bulkNewScanUid() {
  try {
    if (window.crypto && typeof window.crypto.randomUUID === 'function') {
      return 'scan_' + window.crypto.randomUUID().replace(/-/g, '');
    }
  } catch (e) {}
  // Fallback for a browser without randomUUID. Time plus randomness, not time
  // alone: two photos snapped in the same millisecond must not collide, and in
  // Rapid mode they can be.
  return 'scan_' + Date.now().toString(16) + Math.random().toString(16).slice(2, 12);
}

/* Every physical copy on a row, one uid each, in a stable order.

   Lazily repaired rather than asserted, because rows arrive from three places
   (a fresh scan, a retry, a merge) and a row from an older session that was
   resumed has no uid at all. A row that reaches the draft path without an
   identity is the one thing that must not happen, so it is minted here.

   Surplus uids are KEPT when qty drops. If the seller sets qty 3, drafts all
   three, then corrects qty to 2, the third uid stays parked on this row; if
   they put it back to 3 the third copy replays the draft it already has instead
   of creating a fourth. A uid is only ever re-issued to the same card. */
function _bulkCopyUids(r) {
  if (!r) return [];
  if (!r.scanUid) r.scanUid = _bulkNewScanUid();
  if (!Array.isArray(r.copyUids) || !r.copyUids.length) r.copyUids = [r.scanUid];
  if (r.copyUids[0] !== r.scanUid) r.scanUid = r.copyUids[0];
  const qty = (r.qty && r.qty > 1) ? r.qty : 1;
  while (r.copyUids.length < qty) r.copyUids.push(_bulkNewScanUid());
  return r.copyUids;
}

/* The draftable units of a row: one per physical copy, capped at qty. */
function _bulkDraftUnits(r) {
  if (!r || !r.success) return [];
  const qty = (r.qty && r.qty > 1) ? r.qty : 1;
  return _bulkCopyUids(r).slice(0, qty).map((uid, i) => ({
    uid,
    row: r,
    rowId: r.rowId,
    copyIndex: i + 1,
    copies: qty,
    instanceId: 'inst_scan_' + uid,
  }));
}

/* A scan row expressed in the card vocabulary the rest of the app reads.

   ONE mapping, used by both the Collection save and the draft create. Two
...
function _bulkDraftSelectable() {
  return (window._bulkResults || []).filter(r => r && r.success);
}

function _bulkDraftIsSelected(r) {
  return !!(r && r.scanUid && (window._bulkDraftSel || []).indexOf(r.scanUid) !== -1);
}

function _bulkDraftCheckboxHtml(result) {
  _bulkCopyUids(result);
  const on = _bulkDraftIsSelected(result);
  const qty = (result.qty && result.qty > 1) ? result.qty : 1;
  const label = qty > 1
    ? `Select ${_esc(result.cardName)} (${qty} copies, ${qty} drafts)`
    : `Select ${_esc(result.cardName)} for drafting`;
  return '<button role="checkbox" aria-checked="' + (on ? 'true' : 'false') + '"' +
    ' aria-label="' + label + '"' +
    ' onclick="bulkDraftToggleRow(\'' + result.rowId + '\')"' +
    ' data-bulk-draft-check="' + _esc(result.scanUid) + '"' +
    ' style="flex-shrink:0;width:22px;height:22px;padding:0;border-radius:6px;cursor:pointer;font-size:.7rem;font-weight:900;line-height:1;' +
    (on
      ? 'background:rgba(139,92,246,1);border:1px solid rgba(139,92,246,1);color:#fff">✓'
      : 'background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.25);color:transparent">✓') +
    '</button>';
}

function bulkDraftToggleRow(rowId) {
  const r = _bulkFindResult(rowId);
  if (!r || !r.success) return;
  _bulkCopyUids(r);
  const sel = window._bulkDraftSel || (window._bulkDraftSel = []);
  const at = sel.indexOf(r.scanUid);
  if (at === -1) sel.push(r.scanUid); else sel.splice(at, 1);
  _bulkUpdateRow(rowId, r);
  _bulkDraftSyncSelectionUI();
}

function bulkDraftSelectAll() {
  const rows = _bulkDraftSelectable();
  const all = rows.length > 0 && rows.every(r => _bulkDraftIsSelected(r));
  window._bulkDraftSel = all ? [] : rows.map(r => { _bulkCopyUids(r); return r.scanUid; });
  rows.forEach(r => _bulkUpdateRow(r.rowId, r));
  _bulkDraftSyncSelectionUI();
}

/* The select-all control, the visible count, and the button label.

   The count is in CARDS and in DRAFTS when those differ, because with a merged
   qty-3 row they do, and "3 selected" next to a button that creates 5 drafts is
   the kind of quiet mismatch that gets discovered after the fact. */
function _bulkDraftSyncSelectionUI() {
  const rows = _bulkDraftSelectable();
  const chosen = rows.filter(r => _bulkDraftIsSelected(r));
  const units = chosen.reduce((n, r) => n + _bulkDraftUnits(r).length, 0);

  const bar = document.getElementById('bulkDraftSelectBar');
  if (bar) bar.style.display = rows.length ? 'flex' : 'none';

  const box = document.getElementById('bulkDraftSelectAllBox');
  if (box) {
    ...
    box.textContent = all ? '✓' : (some ? '–' : '✓');
    box.style.background = (all || some) ? 'rgba(139,92,246,1)' : 'rgba(255,255,255,.06)';
    box.style.borderColor = (all || some) ? 'rgba(139,92,246,1)' : 'rgba(255,255,255,.25)';
    box.style.color = (all || some) ? '#fff' : 'transparent';
  }

  const countEl = document.getElementById('bulkDraftSelectCount');
  if (countEl) {
    countEl.textContent = chosen.length === 0
      ? 'Select cards to draft'
      : chosen.length + ' of ' + rows.length + ' selected' +
        (units !== chosen.length ? ' · ' + units + ' drafts' : '');
  }

  const btn = document.getElementById('bulkCreateDraftsBtn');
  if (btn) {
    const busy = !!window._bulkDraftBusy;
    btn.disabled = busy || units === 0;
    btn.style.opacity = btn.disabled ? '.45' : '1';
    btn.style.cursor = btn.disabled ? 'default' : 'pointer';
    btn.textContent = busy
      ? 'Creating drafts…'
      : (units === 0 ? 'Create Listing Drafts' : 'Create ' + units + ' Listing Draft' + (units !== 1 ? 's' : ''));
  }

  const note = document.getElementById('bulkDraftHeadroomNote');
  if (note) {
    note.textContent = window._bulkDraftHeadroomNote || '';
    note.style.display = window._bulkDraftHeadroomNote ? '' : 'none';
  }
}

/* ── Per-card outcome, rendered on the card's own row ─────────────────────── */

function _bulkDraftOutcomeHtml(result) {
  const units = _bulkDraftUnits(result);
  if (!units.length) return '';
  const outs = units
    .map(u => ({ u, o: (window._bulkDraftOutcomes || {})[u.uid] }))
    .filter(x => x.o);
  if (!outs.length) return '';
  const tone = {
    working:   'rgba(255,255,255,.45)',
    created:   '#4ade80',
    replayed:  '#4ade80',
    refused:   '#fbbf24',
    skipped:   'rgba(255,255,255,.45)',
    error:     '#f87171',
  };
  return '<div style="margin-top:.3rem;display:flex;flex-direction:column;gap:.15rem">' +
    outs.map(({ u, o }) => {
      const c = tone[o.state] || 'rgba(255,255,255,.45)';
      const prefix = (u.copies > 1) ? ('Copy ' + u.copyIndex + '/' + u.copies + ' — ') : '';
...
function _bulkDraftRecord(unit, outcome) {
  if (!window._bulkDraftOutcomes) window._bulkDraftOutcomes = Object.create(null);
  window._bulkDraftOutcomes[unit.uid] = outcome;
  try { _bulkUpdateRow(unit.rowId, unit.row); } catch (e) {}
}

/* Turn a create result into what this seller's row should say.

   The wording for a refusal is the SERVER's sentence, not a local paraphrase.
   api/drafts.js answers a cap refusal with 409 and a full sentence in `error`;
   re-writing it here would mean two different messages for one condition, and
   the one the seller sees would be the one nobody reviewed. */
function _bulkDraftClassify(out) {
  if (!out || !out.ok) {
    const code = (out && out.code) || '';
    const msg = (out && out.message) || '';
    const atCap = /DRAFT_CAP_REACHED/i.test(String(code)) || /draft limit|CAP/i.test(String(msg)) || /CAP/i.test(String(code));
    if (atCap) {
      return { state: 'refused', atCap: true, code: code || 'DRAFT_CAP_REACHED',
               message: msg || 'Draft limit reached — finish or discard a draft to make room.' };
    }
    if (out && out.status === 410) {
      return { state: 'error', message: msg || 'This draft expired before it could be created — scan the card again.' };
    }
    if (out && out.status === 401) {
      return { state: 'error', message: msg || 'Session expired — sign in again to create drafts.' };
    }
    return { state: 'error', message: msg || 'Draft could not be created.' };
  }
...
/* What the seller has room for RIGHT NOW, as far as the list endpoint knows.

   Returned as `{known:false}` rather than a guess when the read fails or the
   endpoint answers in a degraded mode. An unknown headroom shows no number; a
   fabricated one would be planning advice the seller can act on and we cannot
   stand behind. Either way the batch still runs -- the cap is enforced by the
   create call, and this read has no vote. */
async function _bulkDraftHeadroom() {
  try {
    // The same token helper the create path uses (core.js), not a fifth copy
    // of the Firebase force-refresh dance.
    const token = await _crIdToken();
    if (!token) return { known: false };
    const resp = await fetch('/api/drafts?limit=1', { headers: { Authorization: 'Bearer ' + token } });
    if (!resp.ok) return { known: false };
    const j = await resp.json();
    const cap = Number(j && j.cap);
    const total = Number(j && j.total);
    if (!Number.isFinite(cap) || !Number.isFinite(total)) return { known: false };
    if (j && j.degraded) return { known: false };
    return { known: true, cap, total, free: Math.max(0, cap - total) };
  } catch (e) {
    return { known: false };
  }
}
...
async function bulkCreateSelectedDrafts() {
  if (window._bulkDraftBusy) return;

  // Drafts are server-side and per-seller, so an anonymous batch has nowhere to
  // go. Refused here rather than N times over by the create call.
  if (!window.googleUser) {
    showToast('Sign in to create listing drafts.', 'info');
    return;
  }

  const rows = _bulkDraftSelectable().filter(r => _bulkDraftIsSelected(r));
  if (!rows.length) {
    showToast('Select at least one card first.', 'info');
    return;
  }

  const units = rows.reduce((acc, r) => acc.concat(_bulkDraftUnits(r)), []);

  window._bulkDraftBusy = true;
  window._bulkDraftHeadroomNote = '';
  _bulkDraftSyncSelectionUI();

  // Planning information only. Stated as of-a-moment, and the batch is NOT
  // trimmed to fit it: trimming would refuse cards on the strength of a number
  // that can be stale, when the server may well have had room for them.
  const head = await _bulkDraftHeadroom();
  if (head.known && head.free < units.length) {
    window._bulkDraftHeadroomNote =
      'Room for ' + head.free + ' more draft' + (head.free !== 1 ? 's' : '') +
      ' (' + head.total + ' of ' + head.cap + ' used) — ' + units.length + ' selected. ' +
      'Cards past the limit will say so on their own row.';
  } else if (head.known) {
    window._bulkDraftHeadroomNote =
      'Room for ' + head.free + ' more draft' + (head.free !== 1 ? 's' : '') +
      ' (' + head.total + ' of ' + head.cap + ' used).';
  }
  _bulkDraftSyncSelectionUI();

  let created = 0, replayed = 0, failed = 0, notAttempted = 0;
  let capReached = false;

  try {
    for (const u of units) {
      // The cap was refused earlier in THIS batch. Every remaining card is
      // marked, and named, rather than left with no line at all: silence on a
      // row the seller selected reads as success.
      if (capReached) {
        notAttempted++;
        _bulkDraftRecord(u, {
          state: 'skipped',
          message: 'Not attempted — the draft limit was reached earlier in this batch.',
        });
        continue;
      }
      ...
      const priceSource = (price == null) ? null : (u.row.priceSource === 'comp' ? 'comp' : 'seller');

      let out;
      try {
        out = await _crCreateDraft({
          card,
          instanceId: u.instanceId,
          idemKey: _crCreateIdemKey('sell-scan', u.uid, u.instanceId),
          price,
          priceSource,
          source: 'bulk-scan',
          scanPhoto,
        ...
        out = { ok: false, message: (err && err.message) || 'Draft could not be created.' };
      }

      const o = _bulkDraftClassify(out);
      if (o.atCap) capReached = true;
      if (o.state === 'created') created++;
      else if (o.state === 'replayed') replayed++;
      else failed++;
      _bulkDraftRecord(u, o);
    }
  } finally {
    window._bulkDraftBusy = false;
    _bulkDraftSyncSelectionUI();
  ...
  // The summary counts; the rows say which. Deliberately not the only place a
  // failure appears.
  const parts = [];
  if (created) parts.push(created + ' draft' + (created !== 1 ? 's' : '') + ' created');
  if (replayed) parts.push(replayed + ' already had one');
  if (failed) parts.push(failed + ' could not be created');
  if (notAttempted) parts.push(notAttempted + ' not attempted');
  showToast(
    (parts.join(' · ') || 'Nothing to create') + (failed || notAttempted ? ' — see each card below' : ''),
    (failed || notAttempted) ? 'info' : 'success'
  );

  if (created || replayed) {
    try { window.trackEvent?.('bulk_batch_draft', { created, replayed, failed, notAttempted, selected: units.length }); } catch (e) {}
    _bulkDraftShowDraftsLink();
  }
}

/* After a batch, the way on is Drafts -- the same place the single-card path
   lands. No new review surface is introduced here. */
function _bulkDraftShowDraftsLink() {
  const bar = document.getElementById('bulkBottomBar');
  if (!bar || document.getElementById('bulkGoToDraftsBtn')) return;
  const btn = document.createElement('button');
  btn.id = 'bulkGoToDraftsBtn';
  btn.textContent = '📄 Review Drafts';
  btn.style.cssText = 'width:100%;margin-top:.5rem;padding:.55rem;background:transparent;border:1px solid rgba(139,92,246,.4);color:rgba(196,181,253,1);border-radius:10px;font-weight:700;font-size:.8rem;cursor:pointer';
  btn.onclick = function () { closeBulkScan(); switchView('drafts'); };
  bar.appendChild(btn);
}
...
function _bulkSaveToCollection(successful, costs, skipPricing) {
  ...
  successful.forEach((r, i) => {
    ...
    //
    // The identity half of this row now comes from _bulkScanRowToCard, which
    // the batch-draft path uses too -- one mapping, so a card cannot save as
    // one thing and draft as another. The fields below are the ones that are
...
function _bulkResetBottomBarButtons() {
  ...
  bar.innerHTML = `
    ...
    <div id="bulkDraftHeadroomNote" style="display:none;font-size:.7rem;color:rgba(251,191,36,.9);text-align:center;margin-bottom:.5rem;line-height:1.4"></div>
    ...
    <div style="margin-top:.5rem">
      <button id="bulkCreateDraftsBtn" onclick="bulkCreateSelectedDrafts()" style="width:100%;padding:.6rem;background:rgba(139,92,246,.16);border:1px solid rgba(139,92,246,.55);color:rgba(196,181,253,1);border-radius:10px;font-weight:800;font-size:.82rem;cursor:default;opacity:.45" disabled>Create Listing Drafts</button>
    </div>
    <div style="margin-top:.5rem">
