...
// monthly credit deposits — same as pro_plus_monthly).

import { verifyTokenFlexible } from './_verifyToken.js';

const PLUS_ANNUAL_PRICE_ID = 'price_1TptumFW2YZoedIZPYjcPyoQ'; // $199/yr
...
export default async function handler(req, res) {
  ...
  try {
    ...
    params.append('line_items[0][price]', PLUS_ANNUAL_PRICE_ID);
    params.append('line_items[0][quantity]', '1');
    params.append('customer_email', email);
    params.append('success_url', `${origin}/?plus_annual_success=1&session_id={CHECKOUT_SESSION_ID}`);
    params.append('cancel_url',  `${origin}/?plus_annual_cancelled=1`);
    params.append('subscription_data[metadata][google_sub]', googleSub);
    params.append('subscription_data[metadata][plan]', 'pro_plus_annual');
    params.append('metadata[google_sub]', googleSub);
    params.append('metadata[plan]', 'pro_plus_annual');
    params.append('metadata[type]', 'pro_plus_annual');
    // Show savings vs monthly ($24.99 × 12 = $299.88; annual is $199 → saves ~$100)
    params.append('custom_text[submit][message]', 'Save $100 vs monthly — billed once per year. Cancel anytime.');
    ...
    if (!r.ok) {
      ...
      console.error('Stripe plus annual checkout error:', err);
    ...
    console.error('stripe-plus-annual-checkout error:', e);
