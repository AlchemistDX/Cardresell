...
echo ""
echo "▶ [1/46] Asset fingerprints (referenced bundles named after their bytes)"
...
echo ""
echo "▶ [2/46] Syntax check (all inline <script> blocks)"
...
echo ""
echo "▶ [3/46] Auth stack integrity"
...
echo ""
echo "▶ [4/46] Scan-miss regression checks"
...
echo ""
echo "▶ [5/46] Deeplink + companion links (TCGplayer product URL, eBay sell CTAs)"
...
echo ""
echo "▶ [8/46] Stripe webhook P0 checks"
...
echo ""
echo "▶ [9/46] Launch-audit regressions"
...
echo ""
echo "▶ [10/46] Variant selection (premium-printing bias)"
...
echo ""
echo "▶ [11/46] Sports price guard (host + parallel discipline)"
...
echo ""
echo "▶ [12/46] Quick Pricing + headline price"
...
echo ""
echo "▶ [13/46] Sports parallel matching"
...
echo ""
echo "▶ [14/46] Scanner fastpath + miss-logging"
...
echo ""
echo "▶ [15/46] eBay auth + taxonomy (offline)"
...
echo ""
echo "▶ [16/46] Card identity + SKU golden tests"
...
echo ""
echo "▶ [17/46] Sell entry point eligibility (D1 gate, offline)"
...
echo ""
echo "▶ [18/46] Sell gate: out-of-order responses + wire size (D1, offline)"
...
echo ""
echo "▶ [19/46] Listing packet: title + condition + target-net + metadata (offline)"
...
echo ""
echo "▶ [20/46] Draft index recovery + packet schema version (offline)"
...
echo ""
echo "▶ [21/46] C1 draft store — revisions, tombstones, schema safety (offline)"
...
echo ""
echo "▶ [22/46] C1 draft CRUD end-to-end — create/read/edit/delete (offline)"
...
echo ""
echo "▶ [23/46] C2/C3 draft list + cap — hydration, paging, severity (offline)"
...
if [[ "${DRAFT_KV_LIVE:-0}" == "1" ]]; then
  echo "▶ [24/46] C1 draft persistence against the REAL store"
  ...
  echo "▶ [24/46] C1 real-store pass — SKIPPED (set DRAFT_KV_LIVE=1 + KV_REST_API_* to run)"
fi

if [[ "$LOCAL_ONLY" == "0" ]]; then
  echo ""
  echo "▶ [25/46] Prod endpoint smoke ($BASE)"
  ...
  echo ""
  echo "▶ [25/46] Prod endpoint smoke — SKIPPED (--local)"
fi

echo ""
if [[ "${COND_PILLS_BROWSER:-0}" == "1" ]]; then
  echo "▶ [26/46] Condition-applicability interaction (real browser)"
  ...
  echo "▶ [26/46] Condition-applicability interaction — SKIPPED (set COND_PILLS_BROWSER=1 + serve the site at SITE_BASE to run)"
fi

echo ""
echo "▶ [27/46] D2.1 draft-list readiness (derivation + copy ownership)"
...
echo ""
echo "▶ [28/46] D2.1 draft-list focus parameter (offset resolution + refusal)"
...
echo ""
echo "▶ [29/46] D2.1 drafts screen in a real browser (rows, blockers, stubs, no-nav)"
...
echo "▶ [30/46] D3 draft review screen (fields, fee breakdown, freshness, TRS withholding)"
...
echo "▶ [31/46] Top Rated Plus confirmation is scoped to one listing (4 acceptance states)"
...
echo "▶ [32/46] Theme token contrast meets AA in both themes (runtime-resolved)"
...
echo "▶ [33/46] accuracy.html and the fee model agree on venues and audit dates"
...
echo "▶ [34/46] Review-screen fee rows are a dt/dd list with the withheld pair intact"
...
echo ""
echo "▶ [35/46] SOL-PLAT-007 asset extraction — inline JS/CSS stays split and hashed"
...
echo ""
echo "▶ [36/46] Sol-audit blockers — executes the shipped logic, not its text"
...
echo ""
echo "▶ [37/46] Sol majors — flip and pack"
...
echo ""
echo "▶ [38/46] Sol majors — data durability and tombstones"
...
echo ""
echo "▶ [39/46] Sol majors — scan hygiene"
...
echo ""
echo "▶ [40/46] Sol majors — entitlements and session integrity"
...
echo ""
echo "▶ [41/46] Sol majors — accessibility, mobile targets, honest copy"
...
echo ""
echo "▶ [42/46] SOL-PLAT-011/012/013 — gold text AA, social meta, absolute og:url"
...
echo ""
echo "▶ [43/46] Bulk-scan misfire — two bugs, four fixes"
...
echo ""
echo "▶ [44/46] Bulk row regression — Bulbasaur qualifier"
...
echo ""
echo "▶ [45/46] Bulk row regression — Minun set/variant misfire"
...
echo ""
echo "▶ [46/46] Every suite on disk is invoked or declared; slot numbering is derived"
