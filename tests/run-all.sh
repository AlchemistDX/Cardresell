...
echo ""
echo "▶ [1/58] Asset fingerprints (referenced bundles named after their bytes)"
...
echo ""
echo "▶ [2/58] Syntax check (all inline <script> blocks)"
...
echo ""
echo "▶ [3/58] Auth stack integrity"
...
echo ""
echo "▶ [4/58] Scan-miss regression checks"
...
echo ""
echo "▶ [5/58] Deeplink + companion links (TCGplayer product URL, eBay sell CTAs)"
...
echo ""
echo "▶ [8/58] Stripe webhook P0 checks"
...
echo ""
echo "▶ [9/58] Launch-audit regressions"
...
echo ""
echo "▶ [10/58] Variant selection (premium-printing bias)"
...
echo ""
echo "▶ [11/58] Sports price guard (host + parallel discipline)"
...
echo ""
echo "▶ [12/58] Quick Pricing + headline price"
...
echo ""
echo "▶ [13/58] Sports parallel matching"
...
echo ""
echo "▶ [14/58] Scanner fastpath + miss-logging"
...
echo ""
echo "▶ [15/58] eBay auth + taxonomy (offline)"
...
echo ""
echo "▶ [16/58] Card identity + SKU golden tests"
...
echo ""
echo "▶ [17/58] Sell entry point eligibility (D1 gate, offline)"
...
echo ""
echo "▶ [18/58] Sell gate: out-of-order responses + wire size (D1, offline)"
...
echo ""
echo "▶ [19/58] Listing packet: title + condition + target-net + metadata (offline)"
...
echo ""
echo "▶ [20/58] Draft index recovery + packet schema version (offline)"
...
echo ""
echo "▶ [21/58] C1 draft store — revisions, tombstones, schema safety (offline)"
...
echo ""
echo "▶ [22/58] C1 draft CRUD end-to-end — create/read/edit/delete (offline)"
...
echo ""
echo "▶ [23/58] C2/C3 draft list + cap — hydration, paging, severity (offline)"
...
if [[ "${DRAFT_KV_LIVE:-0}" == "1" ]]; then
  echo "▶ [24/58] C1 draft persistence against the REAL store"
  ...
  echo "▶ [24/58] C1 real-store pass — SKIPPED (set DRAFT_KV_LIVE=1 + KV_REST_API_* to run)"
fi

if [[ "$LOCAL_ONLY" == "0" ]]; then
  echo ""
  echo "▶ [25/58] Prod endpoint smoke ($BASE)"
  ...
  echo ""
  echo "▶ [25/58] Prod endpoint smoke — SKIPPED (--local)"
fi

echo ""
if [[ "${COND_PILLS_BROWSER:-0}" == "1" ]]; then
  echo "▶ [26/58] Condition-applicability interaction (real browser)"
  ...
  echo "▶ [26/58] Condition-applicability interaction — SKIPPED (set COND_PILLS_BROWSER=1 + serve the site at SITE_BASE to run)"
fi

echo ""
echo "▶ [27/58] D2.1 draft-list readiness (derivation + copy ownership)"
...
echo ""
echo "▶ [28/58] D2.1 draft-list focus parameter (offset resolution + refusal)"
...
echo ""
echo "▶ [29/58] D2.1 drafts screen in a real browser (rows, blockers, stubs, no-nav)"
...
echo "▶ [30/58] D3 draft review screen (fields, fee breakdown, freshness, TRS withholding)"
...
echo "▶ [31/58] Top Rated Plus confirmation is scoped to one listing (4 acceptance states)"
...
echo "▶ [32/58] Theme token contrast meets AA in both themes (runtime-resolved)"
...
echo "▶ [33/58] accuracy.html and the fee model agree on venues and audit dates"
...
echo "▶ [34/58] Review-screen fee rows are a dt/dd list with the withheld pair intact"
...
echo ""
echo "▶ [35/58] SOL-PLAT-007 asset extraction — inline JS/CSS stays split and hashed"
...
echo ""
echo "▶ [36/58] Sol-audit blockers — executes the shipped logic, not its text"
...
echo ""
echo "▶ [37/58] Sol majors — flip and pack"
...
echo ""
echo "▶ [38/58] Sol majors — data durability and tombstones"
...
echo ""
echo "▶ [39/58] Sol majors — scan hygiene"
...
echo ""
echo "▶ [40/58] Sol majors — entitlements and session integrity"
...
echo ""
echo "▶ [41/58] Sol majors — accessibility, mobile targets, honest copy"
...
echo ""
echo "▶ [42/58] SOL-PLAT-011/012/013 — gold text AA, social meta, absolute og:url"
...
echo ""
echo "▶ [43/58] Bulk-scan misfire — two bugs, four fixes"
...
echo ""
echo "▶ [44/58] Bulk row regression — Bulbasaur qualifier"
...
echo ""
echo "▶ [45/58] Bulk row regression — Minun set/variant misfire"
...
echo ""
echo "▶ [46/58] Grading upside: net comes from the shared fee model (BIAS-1)"
...
echo ""
echo "▶ [47/58] Payout honesty: signed payout bars + four-state cost records (BIAS-6)"
...
echo ""
echo "▶ [48/58] Decision restatements — shipped copy still matches the decisions of record"
...
echo ""
echo "▶ [49/58] TPL proxy contract (offline, mocked upstream) [CH-3]"
...
echo ""
echo "▶ [50/58] TPL cache + aggregate spending allowance (offline, mocked store) [CH-3/R4]"
...
echo ""
echo "▶ [51/58] eBay notification verification token fails closed [CH-2]"
...
echo "▶ [52/58] Draft lifecycle pointer — generations, reservations, fencing (offline)"
...
echo ""
echo "▶ [53/58] An omitted generation is legacy generation 0, then compared like any other"
...
echo ""
echo "▶ [54/58] Draft delete in a real browser — 410 shows the deleted state, no auto-retry"
...
echo ""
echo "▶ [55/58] TPL outcome rendering"
...
echo ""
echo "▶ [56/58] Collection escaping: hostile values stay inside their attributes"
...
echo ""
echo "▶ [57/58] Draft card actions in a real browser — edit, delete, download, desktop and 375px"
...
echo ""
echo "▶ [58/58] Every suite on disk is invoked or declared; slot numbering is derived"
