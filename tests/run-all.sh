...
echo ""
echo "▶ [1/64] Asset fingerprints (referenced bundles named after their bytes)"
...
echo ""
echo "▶ [2/64] Syntax check (all inline <script> blocks)"
...
echo ""
echo "▶ [3/64] Auth stack integrity"
...
echo ""
echo "▶ [4/64] Scan-miss regression checks"
...
echo ""
echo "▶ [5/64] Deeplink + companion links (TCGplayer product URL, eBay sell CTAs)"
...
echo ""
echo "▶ [8/64] Stripe webhook P0 checks"
...
echo ""
echo "▶ [9/64] Launch-audit regressions"
...
echo ""
echo "▶ [10/64] Variant selection (premium-printing bias)"
...
echo ""
echo "▶ [11/64] Sports price guard (host + parallel discipline)"
...
echo ""
echo "▶ [12/64] Quick Pricing + headline price"
...
echo ""
echo "▶ [13/64] Sports parallel matching"
...
echo ""
echo "▶ [14/64] Scanner fastpath + miss-logging"
...
echo ""
echo "▶ [15/64] eBay auth + taxonomy (offline)"
...
echo ""
echo "▶ [16/64] Card identity + SKU golden tests"
...
echo ""
echo "▶ [17/64] Sell entry point eligibility (D1 gate, offline)"
...
echo ""
echo "▶ [18/64] Sell gate: out-of-order responses + wire size (D1, offline)"
...
echo ""
echo "▶ [19/64] Listing packet: title + condition + target-net + metadata (offline)"
...
echo ""
echo "▶ [20/64] Draft index recovery + packet schema version (offline)"
...
echo ""
echo "▶ [21/64] C1 draft store — revisions, tombstones, schema safety (offline)"
...
echo ""
echo "▶ [22/64] C1 draft CRUD end-to-end — create/read/edit/delete (offline)"
...
echo ""
echo "▶ [23/64] C2/C3 draft list + cap — hydration, paging, severity (offline)"
...
if [[ "${DRAFT_KV_LIVE:-0}" == "1" ]]; then
  echo "▶ [24/64] C1 draft persistence against the REAL store"
  ...
  echo "▶ [24/64] C1 real-store pass — SKIPPED (set DRAFT_KV_LIVE=1 + KV_REST_API_* to run)"
fi

if [[ "$LOCAL_ONLY" == "0" ]]; then
  echo ""
  echo "▶ [25/64] Prod endpoint smoke ($BASE)"
  ...
  echo ""
  echo "▶ [25/64] Prod endpoint smoke — SKIPPED (--local)"
fi

echo ""
if [[ "${COND_PILLS_BROWSER:-0}" == "1" ]]; then
  echo "▶ [26/64] Condition-applicability interaction (real browser)"
  ...
  echo "▶ [26/64] Condition-applicability interaction — SKIPPED (set COND_PILLS_BROWSER=1 + serve the site at SITE_BASE to run)"
fi

echo ""
echo "▶ [27/64] D2.1 draft-list readiness (derivation + copy ownership)"
...
echo ""
echo "▶ [28/64] D2.1 draft-list focus parameter (offset resolution + refusal)"
...
echo ""
echo "▶ [29/64] D2.1 drafts screen in a real browser (rows, blockers, stubs, no-nav)"
...
echo "▶ [30/64] D3 draft review screen (fields, fee breakdown, freshness, TRS withholding)"
...
echo "▶ [31/64] Top Rated Plus confirmation is scoped to one listing (4 acceptance states)"
...
echo "▶ [32/64] Theme token contrast meets AA in both themes (runtime-resolved)"
...
echo "▶ [33/64] accuracy.html and the fee model agree on venues and audit dates"
...
echo "▶ [34/64] Review-screen fee rows are a dt/dd list with the withheld pair intact"
...
echo ""
echo "▶ [35/64] SOL-PLAT-007 asset extraction — inline JS/CSS stays split and hashed"
...
echo ""
echo "▶ [36/64] Sol-audit blockers — executes the shipped logic, not its text"
...
echo ""
echo "▶ [37/64] Sol majors — flip and pack"
...
echo ""
echo "▶ [38/64] Sol majors — data durability and tombstones"
...
echo ""
echo "▶ [39/64] Sol majors — scan hygiene"
...
echo ""
echo "▶ [40/64] Sol majors — entitlements and session integrity"
...
echo ""
echo "▶ [41/64] Sol majors — accessibility, mobile targets, honest copy"
...
echo ""
echo "▶ [42/64] SOL-PLAT-011/012/013 — gold text AA, social meta, absolute og:url"
...
echo ""
echo "▶ [43/64] Bulk-scan misfire — two bugs, four fixes"
...
echo ""
echo "▶ [44/64] Bulk row regression — Bulbasaur qualifier"
...
echo ""
echo "▶ [45/64] Bulk row regression — Minun set/variant misfire"
...
echo ""
echo "▶ [46/64] Grading upside: net comes from the shared fee model (BIAS-1)"
...
echo ""
echo "▶ [47/64] Payout honesty: signed payout bars + four-state cost records (BIAS-6)"
...
echo ""
echo "▶ [48/64] Decision restatements — shipped copy still matches the decisions of record"
...
echo ""
echo "▶ [49/64] TPL proxy contract (offline, mocked upstream) [CH-3]"
...
echo ""
echo "▶ [50/64] TPL cache + aggregate spending allowance (offline, mocked store) [CH-3/R4]"
...
echo ""
echo "▶ [51/64] eBay notification verification token fails closed [CH-2]"
...
echo "▶ [52/64] Draft lifecycle pointer — generations, reservations, fencing (offline)"
...
echo ""
echo "▶ [53/64] An omitted generation is legacy generation 0, then compared like any other"
...
echo ""
echo "▶ [54/64] Draft delete in a real browser — 410 shows the deleted state, no auto-retry"
...
echo ""
echo "▶ [55/64] TPL outcome rendering"
...
echo ""
echo "▶ [56/64] Collection escaping: hostile values stay inside their attributes"
...
echo ""
echo "▶ [57/64] Draft card actions in a real browser — edit, delete, download, desktop and 375px"
...
echo ""
echo "▶ [58/64] Batch drafting from Bulk/Rapid Scan in a real browser — scan identity, copies, retry, cap refusal"
...
echo ""
echo "▶ [59/64] Collector number display — denominator safeguards, prefixes, leading zeros, SKU-neutrality"
...
echo ""
echo "▶ [60/64] Scan rarity grounding — the grounded record overrides an invented rarity"
...
echo ""
echo "▶ [61/64] Cert generation matcher — no mislabelled generation, no entered value in output"
...
echo ""
echo "▶ [62/64] eBay credential checker output — no supplied credential or token on any path"
...
echo ""
echo "▶ [63/64] Seeder collector numbers — leading zeros and prefixes survive verbatim"
...
echo ""
echo "▶ [64/64] Every suite on disk is invoked or declared; slot numbering is derived"
