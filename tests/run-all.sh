...
echo ""
echo "▶ [1/47] Asset fingerprints (referenced bundles named after their bytes)"
...
echo ""
echo "▶ [2/47] Syntax check (all inline <script> blocks)"
...
echo ""
echo "▶ [3/47] Auth stack integrity"
...
echo ""
echo "▶ [4/47] Scan-miss regression checks"
...
echo ""
echo "▶ [5/47] Deeplink + companion links (TCGplayer product URL, eBay sell CTAs)"
...
echo ""
echo "▶ [8/47] Stripe webhook P0 checks"
...
echo ""
echo "▶ [9/47] Launch-audit regressions"
...
echo ""
echo "▶ [10/47] Variant selection (premium-printing bias)"
...
echo ""
echo "▶ [11/47] Sports price guard (host + parallel discipline)"
...
echo ""
echo "▶ [12/47] Quick Pricing + headline price"
...
echo ""
echo "▶ [13/47] Sports parallel matching"
...
echo ""
echo "▶ [14/47] Scanner fastpath + miss-logging"
...
echo ""
echo "▶ [15/47] eBay auth + taxonomy (offline)"
...
echo ""
echo "▶ [16/47] Card identity + SKU golden tests"
...
echo ""
echo "▶ [17/47] Sell entry point eligibility (D1 gate, offline)"
...
echo ""
echo "▶ [18/47] Sell gate: out-of-order responses + wire size (D1, offline)"
...
echo ""
echo "▶ [19/47] Listing packet: title + condition + target-net + metadata (offline)"
...
echo ""
echo "▶ [20/47] Draft index recovery + packet schema version (offline)"
...
echo ""
echo "▶ [21/47] C1 draft store — revisions, tombstones, schema safety (offline)"
...
echo ""
echo "▶ [22/47] C1 draft CRUD end-to-end — create/read/edit/delete (offline)"
...
echo ""
echo "▶ [23/47] C2/C3 draft list + cap — hydration, paging, severity (offline)"
...
if [[ "${DRAFT_KV_LIVE:-0}" == "1" ]]; then
  echo "▶ [24/47] C1 draft persistence against the REAL store"
  ...
  echo "▶ [24/47] C1 real-store pass — SKIPPED (set DRAFT_KV_LIVE=1 + KV_REST_API_* to run)"
fi

if [[ "$LOCAL_ONLY" == "0" ]]; then
  echo ""
  echo "▶ [25/47] Prod endpoint smoke ($BASE)"
  ...
  echo ""
  echo "▶ [25/47] Prod endpoint smoke — SKIPPED (--local)"
fi

echo ""
if [[ "${COND_PILLS_BROWSER:-0}" == "1" ]]; then
  echo "▶ [26/47] Condition-applicability interaction (real browser)"
  ...
  echo "▶ [26/47] Condition-applicability interaction — SKIPPED (set COND_PILLS_BROWSER=1 + serve the site at SITE_BASE to run)"
fi

echo ""
echo "▶ [27/47] D2.1 draft-list readiness (derivation + copy ownership)"
...
echo ""
echo "▶ [28/47] D2.1 draft-list focus parameter (offset resolution + refusal)"
...
echo ""
echo "▶ [29/47] D2.1 drafts screen in a real browser (rows, blockers, stubs, no-nav)"
...
echo "▶ [30/47] D3 draft review screen (fields, fee breakdown, freshness, TRS withholding)"
...
echo "▶ [31/47] Top Rated Plus confirmation is scoped to one listing (4 acceptance states)"
...
echo "▶ [32/47] Theme token contrast meets AA in both themes (runtime-resolved)"
...
echo "▶ [33/47] accuracy.html and the fee model agree on venues and audit dates"
...
echo "▶ [34/47] Review-screen fee rows are a dt/dd list with the withheld pair intact"
...
echo ""
echo "▶ [35/47] SOL-PLAT-007 asset extraction — inline JS/CSS stays split and hashed"
...
echo ""
echo "▶ [36/47] Sol-audit blockers — executes the shipped logic, not its text"
...
echo ""
echo "▶ [37/47] Sol majors — flip and pack"
...
echo ""
echo "▶ [38/47] Sol majors — data durability and tombstones"
...
echo ""
echo "▶ [39/47] Sol majors — scan hygiene"
...
echo ""
echo "▶ [40/47] Sol majors — entitlements and session integrity"
...
echo ""
echo "▶ [41/47] Sol majors — accessibility, mobile targets, honest copy"
...
echo ""
echo "▶ [42/47] SOL-PLAT-011/012/013 — gold text AA, social meta, absolute og:url"
...
echo ""
echo "▶ [43/47] Bulk-scan misfire — two bugs, four fixes"
...
echo ""
echo "▶ [44/47] Bulk row regression — Bulbasaur qualifier"
...
echo ""
echo "▶ [45/47] Bulk row regression — Minun set/variant misfire"
...
echo ""
echo "▶ [46/47] Grading upside: net comes from the shared fee model (BIAS-1)"
...
echo ""
echo "▶ [47/47] Every suite on disk is invoked or declared; slot numbering is derived"
