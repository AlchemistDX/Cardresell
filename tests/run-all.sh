...
echo ""
echo "▶ [1/74] Asset fingerprints (referenced bundles named after their bytes)"
...
echo ""
echo "▶ [2/74] Syntax check (all inline <script> blocks)"
...
echo ""
echo "▶ [3/74] Auth stack integrity"
...
echo ""
echo "▶ [4/74] Scan-miss regression checks"
...
echo ""
echo "▶ [5/74] Deeplink + companion links (TCGplayer product URL, eBay sell CTAs)"
...
echo ""
echo "▶ [8/74] Stripe webhook P0 checks"
...
echo ""
echo "▶ [9/74] Launch-audit regressions"
...
echo ""
echo "▶ [10/74] Variant selection (premium-printing bias)"
...
echo ""
echo "▶ [11/74] Sports price guard (host + parallel discipline)"
...
echo ""
echo "▶ [12/74] Quick Pricing + headline price"
...
echo ""
echo "▶ [13/74] Sports parallel matching"
...
echo ""
echo "▶ [14/74] Scanner fastpath + miss-logging"
...
echo ""
echo "▶ [15/74] eBay auth + taxonomy (offline)"
...
echo ""
echo "▶ [16/74] Card identity + SKU golden tests"
...
echo ""
echo "▶ [17/74] Sell entry point eligibility (D1 gate, offline)"
...
echo ""
echo "▶ [18/74] Sell gate: out-of-order responses + wire size (D1, offline)"
...
echo ""
echo "▶ [19/74] Listing packet: title + condition + target-net + metadata (offline)"
...
echo ""
echo "▶ [20/74] Draft index recovery + packet schema version (offline)"
...
echo ""
echo "▶ [21/74] C1 draft store — revisions, tombstones, schema safety (offline)"
...
echo ""
echo "▶ [22/74] C1 draft CRUD end-to-end — create/read/edit/delete (offline)"
...
echo ""
echo "▶ [23/74] C2/C3 draft list + cap — hydration, paging, severity (offline)"
...
if [[ "${DRAFT_KV_LIVE:-0}" == "1" ]]; then
  echo "▶ [24/74] C1 draft persistence against the REAL store"
  ...
  echo "▶ [24/74] C1 real-store pass — SKIPPED (set DRAFT_KV_LIVE=1 + KV_REST_API_* to run)"
fi

if [[ "$LOCAL_ONLY" == "0" ]]; then
  echo ""
  echo "▶ [25/74] Prod endpoint smoke ($BASE)"
  ...
  echo ""
  echo "▶ [25/74] Prod endpoint smoke — SKIPPED (--local)"
fi

echo ""
if [[ "${COND_PILLS_BROWSER:-0}" == "1" ]]; then
  echo "▶ [26/74] Condition-applicability interaction (real browser)"
  ...
  echo "▶ [26/74] Condition-applicability interaction — SKIPPED (set COND_PILLS_BROWSER=1 + serve the site at SITE_BASE to run)"
fi

echo ""
echo "▶ [27/74] D2.1 draft-list readiness (derivation + copy ownership)"
...
echo ""
echo "▶ [28/74] D2.1 draft-list focus parameter (offset resolution + refusal)"
...
echo ""
echo "▶ [29/74] D2.1 drafts screen in a real browser (rows, blockers, stubs, no-nav)"
...
echo "▶ [30/74] D3 draft review screen (fields, fee breakdown, freshness, TRS withholding)"
...
echo "▶ [31/74] Top Rated Plus confirmation is scoped to one listing (4 acceptance states)"
...
echo "▶ [32/74] Theme token contrast meets AA in both themes (runtime-resolved)"
...
echo "▶ [33/74] accuracy.html and the fee model agree on venues and audit dates"
...
echo "▶ [34/74] Review-screen fee rows are a dt/dd list with the withheld pair intact"
...
echo ""
echo "▶ [35/74] SOL-PLAT-007 asset extraction — inline JS/CSS stays split and hashed"
...
echo ""
echo "▶ [36/74] Sol-audit blockers — executes the shipped logic, not its text"
...
echo ""
echo "▶ [37/74] Sol majors — flip and pack"
...
echo ""
echo "▶ [38/74] Sol majors — data durability and tombstones"
...
echo ""
echo "▶ [39/74] Sol majors — scan hygiene"
...
echo ""
echo "▶ [40/74] Sol majors — entitlements and session integrity"
...
echo ""
echo "▶ [41/74] Sol majors — accessibility, mobile targets, honest copy"
...
echo ""
echo "▶ [42/74] SOL-PLAT-011/012/013 — gold text AA, social meta, absolute og:url"
...
echo ""
echo "▶ [43/74] Bulk-scan misfire — two bugs, four fixes"
...
echo ""
echo "▶ [44/74] Bulk row regression — Bulbasaur qualifier"
...
echo ""
echo "▶ [45/74] Bulk row regression — Minun set/variant misfire"
...
echo ""
echo "▶ [46/74] Grading upside: net comes from the shared fee model (BIAS-1)"
...
echo ""
echo "▶ [47/74] Payout honesty: signed payout bars + four-state cost records (BIAS-6)"
...
echo ""
echo "▶ [48/74] Decision restatements — shipped copy still matches the decisions of record"
...
echo ""
echo "▶ [49/74] TPL proxy contract (offline, mocked upstream) [CH-3]"
...
echo ""
echo "▶ [50/74] TPL cache + aggregate spending allowance (offline, mocked store) [CH-3/R4]"
...
echo ""
echo "▶ [51/74] eBay notification verification token fails closed [CH-2]"
...
echo "▶ [52/74] Draft lifecycle pointer — generations, reservations, fencing (offline)"
...
echo ""
echo "▶ [53/74] An omitted generation is legacy generation 0, then compared like any other"
...
echo ""
echo "▶ [54/74] Draft delete in a real browser — 410 shows the deleted state, no auto-retry"
...
echo ""
echo "▶ [55/74] TPL outcome rendering"
...
echo ""
echo "▶ [56/74] Collection escaping: hostile values stay inside their attributes"
...
echo ""
echo "▶ [57/74] Draft card actions in a real browser — edit, delete, download, desktop and 375px"
...
echo ""
echo "▶ [58/74] Batch drafting from Bulk/Rapid Scan in a real browser — scan identity, copies, retry, cap refusal"
...
echo ""
echo "▶ [59/74] Collector number display — denominator safeguards, prefixes, leading zeros, SKU-neutrality"
...
echo ""
echo "▶ [60/74] Scan rarity grounding — the grounded record overrides an invented rarity"
...
echo ""
echo "▶ [61/74] Cert generation matcher — no mislabelled generation, no entered value in output"
...
echo ""
echo "▶ [62/74] eBay credential checker output — no supplied credential or token on any path"
...
echo ""
echo "▶ [63/74] Seeder collector numbers — leading zeros and prefixes survive verbatim"
...
echo ""
echo "▶ [64/74] Printed denominator resolves without the filesystem; embedded table matches the seeder record"
...
echo ""
echo "▶ [65/74] Photo hosting contract — eBay’s documented photo-column limits, and no host configured yet"
...
echo ""
echo "▶ [66/74] Photo upload endpoint — a non-owner cannot host another seller’s draft photos"
...
echo ""
echo "▶ [67/74] Item photo URL is written, and a blank column is disclosed rather than claimed as sent"
...
echo ""
echo "▶ [68/74] Draft-list Download button: seller photo lands or the file is refused (never blank)"
...
echo ""
echo "▶ [69/74] Identity resolver in isolation: an EXACT_MATCH requires positive candidate agreement"
if suite "$ROOT/tests/identity-resolution.mjs"; then
  :
else
  FAIL=1
fi
...
echo ""
echo "▶ [70/74] Identity wiring: all four grounding call sites route through the resolver"
if suite "$ROOT/tests/identity-wiring.mjs"; then
  :
else
  FAIL=1
fi
...
echo ""
echo "▶ [71/74] Real /api/scan handler: ambiguity reaches NEEDS_CONFIRMATION, never an automatic answer"
if suite "$ROOT/tests/scan-handler-integration.mjs"; then
  :
else
  FAIL=1
fi
...
echo ""
echo "▶ [72/74] Two-pass migration collision plan: a contested identity blocks BOTH sides"
if suite "$ROOT/tools/migration/ptcgoCodeResolve.test.mjs"; then
  :
else
  FAIL=1
fi
...
echo ""
echo "▶ [73/74] Every suite on disk is invoked or declared; slot numbering is derived"
...
echo ""
# Requires redis-server, redis-cli and the redis Node client. Starts a private
# local socket, executes exact production Lua, never uses a live KV endpoint.
echo "▶ [74/74] Atomic ID confirmation billing — real handlers and isolated Redis Lua"
if suite "$ROOT/tests/id-confirmation-atomic.mjs"; then
  :
else
  FAIL=1
fi
