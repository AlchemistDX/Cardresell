...
echo ""
echo "▶ [1/33] Asset fingerprints (referenced bundles named after their bytes)"
...
echo ""
echo "▶ [2/33] Syntax check (all inline <script> blocks)"
...
echo ""
echo "▶ [3/33] Auth stack integrity"
...
echo ""
echo "▶ [4/33] Scan-miss regression checks"
...
echo ""
echo "▶ [5/33] Deeplink + companion links (TCGplayer product URL, eBay sell CTAs)"
...
echo ""
echo "▶ [8/33] Stripe webhook P0 checks"
...
echo ""
echo "▶ [9/33] Launch-audit regressions"
...
echo ""
echo "▶ [10/33] Variant selection (premium-printing bias)"
...
echo ""
echo "▶ [11/33] Sports price guard (host + parallel discipline)"
...
echo ""
echo "▶ [12/33] Quick Pricing + headline price"
...
echo ""
echo "▶ [13/33] Sports parallel matching"
...
echo ""
echo "▶ [14/33] Scanner fastpath + miss-logging"
...
echo ""
echo "▶ [15/33] eBay auth + taxonomy (offline)"
...
echo ""
echo "▶ [16/33] Card identity + SKU golden tests"
...
echo ""
echo "▶ [17/33] Sell entry point eligibility (D1 gate, offline)"
...
echo ""
echo "▶ [18/33] Sell gate: out-of-order responses + wire size (D1, offline)"
...
echo ""
echo "▶ [19/33] Listing packet: title + condition + target-net + metadata (offline)"
...
echo ""
echo "▶ [20/33] Draft index recovery + packet schema version (offline)"
...
echo ""
echo "▶ [21/33] C1 draft store — revisions, tombstones, schema safety (offline)"
...
echo ""
echo "▶ [22/33] C1 draft CRUD end-to-end — create/read/edit/delete (offline)"
...
echo ""
echo "▶ [23/33] C2/C3 draft list + cap — hydration, paging, severity (offline)"
...
if [[ "${DRAFT_KV_LIVE:-0}" == "1" ]]; then
  echo "▶ [24/33] C1 draft persistence against the REAL store"
  ...
  echo "▶ [24/33] C1 real-store pass — SKIPPED (set DRAFT_KV_LIVE=1 + KV_REST_API_* to run)"
fi

if [[ "$LOCAL_ONLY" == "0" ]]; then
  echo ""
  echo "▶ [25/33] Prod endpoint smoke ($BASE)"
  ...
  echo ""
  echo "▶ [25/33] Prod endpoint smoke — SKIPPED (--local)"
fi

echo ""
if [[ "${COND_PILLS_BROWSER:-0}" == "1" ]]; then
  echo "▶ [26/33] Condition-applicability interaction (real browser)"
  ...
  echo "▶ [26/33] Condition-applicability interaction — SKIPPED (set COND_PILLS_BROWSER=1 + serve the site at SITE_BASE to run)"
fi

echo ""
echo "▶ [27/33] D2.1 draft-list readiness (derivation + copy ownership)"
...
echo ""
echo "▶ [28/33] D2.1 draft-list focus parameter (offset resolution + refusal)"
...
echo ""
echo "▶ [29/33] D2.1 drafts screen in a real browser (rows, blockers, stubs, no-nav)"
...
echo "▶ [30/33] D3 draft review screen (fields, fee breakdown, freshness, TRS withholding)"
...
echo "▶ [31/33] Top Rated Plus confirmation is scoped to one listing (4 acceptance states)"
...
echo "▶ [32/33] Theme token contrast meets AA in both themes (runtime-resolved)"
...
echo "▶ [33/33] accuracy.html and the fee model agree on venues and audit dates"
