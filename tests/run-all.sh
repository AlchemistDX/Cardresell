...
echo ""
echo "▶ [1/50] Asset fingerprints (referenced bundles named after their bytes)"
...
echo ""
echo "▶ [2/50] Syntax check (all inline <script> blocks)"
...
echo ""
echo "▶ [3/50] Auth stack integrity"
...
echo ""
echo "▶ [4/50] Scan-miss regression checks"
...
echo ""
echo "▶ [5/50] Deeplink + companion links (TCGplayer product URL, eBay sell CTAs)"
...
echo ""
echo "▶ [8/50] Stripe webhook P0 checks"
...
echo ""
echo "▶ [9/50] Launch-audit regressions"
...
echo ""
echo "▶ [10/50] Variant selection (premium-printing bias)"
...
echo ""
echo "▶ [11/50] Sports price guard (host + parallel discipline)"
...
echo ""
echo "▶ [12/50] Quick Pricing + headline price"
...
echo ""
echo "▶ [13/50] Sports parallel matching"
...
echo ""
echo "▶ [14/50] Scanner fastpath + miss-logging"
...
echo ""
echo "▶ [15/50] eBay auth + taxonomy (offline)"
...
echo ""
echo "▶ [16/50] Card identity + SKU golden tests"
...
echo ""
echo "▶ [17/50] Sell entry point eligibility (D1 gate, offline)"
...
echo ""
echo "▶ [18/50] Sell gate: out-of-order responses + wire size (D1, offline)"
...
echo ""
echo "▶ [19/50] Listing packet: title + condition + target-net + metadata (offline)"
...
echo ""
echo "▶ [20/50] Draft index recovery + packet schema version (offline)"
...
echo ""
echo "▶ [21/50] C1 draft store — revisions, tombstones, schema safety (offline)"
...
echo ""
echo "▶ [22/50] C1 draft CRUD end-to-end — create/read/edit/delete (offline)"
...
echo ""
echo "▶ [23/50] C2/C3 draft list + cap — hydration, paging, severity (offline)"
...
if [[ "${DRAFT_KV_LIVE:-0}" == "1" ]]; then
  echo "▶ [24/50] C1 draft persistence against the REAL store"
  ...
  echo "▶ [24/50] C1 real-store pass — SKIPPED (set DRAFT_KV_LIVE=1 + KV_REST_API_* to run)"
fi

if [[ "$LOCAL_ONLY" == "0" ]]; then
  echo ""
  echo "▶ [25/50] Prod endpoint smoke ($BASE)"
  ...
  echo ""
  echo "▶ [25/50] Prod endpoint smoke — SKIPPED (--local)"
fi

echo ""
if [[ "${COND_PILLS_BROWSER:-0}" == "1" ]]; then
  echo "▶ [26/50] Condition-applicability interaction (real browser)"
  ...
  echo "▶ [26/50] Condition-applicability interaction — SKIPPED (set COND_PILLS_BROWSER=1 + serve the site at SITE_BASE to run)"
fi

echo ""
echo "▶ [27/50] D2.1 draft-list readiness (derivation + copy ownership)"
...
echo ""
echo "▶ [28/50] D2.1 draft-list focus parameter (offset resolution + refusal)"
...
echo ""
echo "▶ [29/50] D2.1 drafts screen in a real browser (rows, blockers, stubs, no-nav)"
...
echo "▶ [30/50] D3 draft review screen (fields, fee breakdown, freshness, TRS withholding)"
...
echo "▶ [31/50] Top Rated Plus confirmation is scoped to one listing (4 acceptance states)"
...
echo "▶ [32/50] Theme token contrast meets AA in both themes (runtime-resolved)"
...
echo "▶ [33/50] accuracy.html and the fee model agree on venues and audit dates"
...
echo "▶ [34/50] Review-screen fee rows are a dt/dd list with the withheld pair intact"
...
echo ""
echo "▶ [35/50] SOL-PLAT-007 asset extraction — inline JS/CSS stays split and hashed"
...
echo ""
echo "▶ [36/50] Sol-audit blockers — executes the shipped logic, not its text"
...
echo ""
echo "▶ [37/50] Sol majors — flip and pack"
...
echo ""
echo "▶ [38/50] Sol majors — data durability and tombstones"
...
echo ""
echo "▶ [39/50] Sol majors — scan hygiene"
...
echo ""
echo "▶ [40/50] Sol majors — entitlements and session integrity"
...
echo ""
echo "▶ [41/50] Sol majors — accessibility, mobile targets, honest copy"
...
echo ""
echo "▶ [42/50] SOL-PLAT-011/012/013 — gold text AA, social meta, absolute og:url"
...
echo ""
echo "▶ [43/50] Bulk-scan misfire — two bugs, four fixes"
...
echo ""
echo "▶ [44/50] Bulk row regression — Bulbasaur qualifier"
...
echo ""
echo "▶ [45/50] Bulk row regression — Minun set/variant misfire"
...
echo ""
echo "▶ [46/50] Grading upside: net comes from the shared fee model (BIAS-1)"
...
echo ""
echo "▶ [47/50] Payout honesty: signed payout bars + four-state cost records (BIAS-6)"
...
echo ""
echo "▶ [48/50] Decision restatements — shipped copy still matches the decisions of record"
...
echo ""
echo "▶ [49/50] TPL proxy contract (offline, mocked upstream) [CH-3]"
...
echo ""
echo "▶ [50/50] Every suite on disk is invoked or declared; slot numbering is derived"
