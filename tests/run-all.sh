...
echo ""
echo "▶ [1/3] Syntax check (all inline <script> blocks)"
...
if [[ "$LOCAL_ONLY" == "0" ]]; then
  echo ""
  echo "▶ [3/3] Prod endpoint smoke ($BASE)"
  ...
  echo ""
  echo "▶ [3/3] Prod endpoint smoke — SKIPPED (--local)"
fi
