...
const candidate = payload => payload.identity_resolution.all_candidates[6];
  ...
  try {
    ...
    if (row.action === 'deny-selection') {
      ...
      t.check(`${label}: another actual scan spends the last free credit`,
        other.statusCode === 200 && other.payload.identified === true
...
}
{
  ...
  const conflict = await h.pick({ ...body, candidate: initial.payload.identity_resolution.all_candidates[0] });
...
}
{
  ...
  const b2 = { ...b1, candidate: initial.payload.identity_resolution.all_candidates[0] };
  const results = await Promise.all([h.pick(b1), h.pick(b2)]);
