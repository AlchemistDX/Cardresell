...
let store, commands, fault, now, savedRaw, checkpointBeforeNext, lastPhase, probes;
function reset() {
  store = redisStore(); commands = []; fault = {}; savedRaw = new Map();
  checkpointBeforeNext = []; probes = [];
  lastPhase = null; now = Date.parse('2026-09-15T18:00:00Z');
...
globalThis.fetch = async (url, init) => {
  ...
  if (fault.probe && probe) throw new Error(CANARY);
  if (fault.rest && a[0] === 'GET' && a[1].startsWith('id_billing:')) throw new Error(CANARY);
  if (billing) {
    if (lastPhase) {
...
try {
  ...
  for (const mode of ['rest', 'probe', 'checkpoint', 'finish', 'cleanup', 'lostAccept', 'advance']) {
    ...
    if (mode === 'lostAccept') t.check('lost committed module response captures accepted journal before cleanup',
      e.captures.length === 3 && e.captures[2].lua.nested.valid && e.error === 'billing_unavailable');
    fault = {};
